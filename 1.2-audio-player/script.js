document.addEventListener("DOMContentLoaded", function () {
  ...
  // Функция для загрузки трека и отображения информации о нем
  const loadAndDisplayTrackInfo = (trackIndex) => {
  ...
  nextButton.addEventListener("click", () => {
    ...
    loadAndDisplayTrackInfo(currentTrackIndex);
  });
  ...
  audio.addEventListener("ended", () => {
    if (isRepeatOne) {
      ...
      nextButton.click(); // Автоматическое переключение на следующий трек
    }
  });

  // Загрузка информации о первом треке при загрузке страницы
  loadAndDisplayTrackInfo(currentTrackIndex);
});
